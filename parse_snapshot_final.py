...
# Глобальная переменная для отслеживания номера дисклеймера
DISCLAIMER_COUNTER_FILE = "disclaimer_counter.txt"

def get_next_disclaimer():
    """Получает следующий дисклеймер из списка, циклически чередуя"""
    try:
        with open("disclaimers.txt", "r", encoding="utf-8") as f:
            disclaimers = [line.strip() for line in f if line.strip()]
    except:
        disclaimers = ["⚠️ Беттинг связан с рисками. Анализируйте самостоятельно перед ставкой."]
...
def clean_team_name(name):
    """Очищает название команды от мусора (счётов и т.д.)"""
    # Удаляем "0 0 0 0" и подобные паттерны в конце
    name = re.sub(r'\s+\d+\s+\d+\s+\d+\s+\d+\s*$', '', name).strip()
    # Удаляем "(ж)" если это конец строки для женских команд
    name = re.sub(r'\s*\(ж\)\s*$', '', name).strip()
    return name

def extract_matches_from_snapshot(snapshot_text):
    """Парсит snapshot и извлекает матчи"""
    matches = []
    ...
    while i < len(lines):
        ...
        # Ищем текст первой команды (кириллица)
        if '- text:' in line and any(c in 'АБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ' for c in line):
            ...
            # Пропускаем названия лиг, фильтры и служебные текст
            if any(x in team1 for x in ['Премьер', 'Лига', 'Серия', 'Бундеслига', 'Примера', 'Исход', 'Тотал', 'Фора', 'live', 'Оценка', 'icon']):
                i += 1
                continue
            ...
            while j < min(i + 20, len(lines)):
                ...
                if team2_match:
                    ...
                    # Пропускаем служебные текст
                    if not any(x in team2_candidate for x in ['Премьер', 'Лига', 'Серия', 'Бундеслига', 'live', 'icon', '+', '1', '2', '3']):
                        team2 = team2_candidate
                        score_p1 = int(team2_match.group(2))
                        score_p2 = int(team2_match.group(3))
                        
                        # Ищем время и коэффициенты
                        time_str = ""
                        coef_p1 = None
                        coef_x = None
                        coef_p2 = None
...
def format_telegram_message(recommendations):
    ...
    # Добавляем случайный дисклеймер из файла
...
def main():
    ...
    if matches:
        print("📊 НАЙДЕННЫЕ МАТЧИ (первые 5):")
        for m in matches[:5]:
