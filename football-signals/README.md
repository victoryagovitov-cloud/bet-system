...
- `MIN_EDGE`…`MAX_EDGE` (по умолчанию 2%…12%) — слишком жирный edge = недоверие к модели;
...
- LLM logic — **стоп-кран** (скептик): те же правила + контекст 1X2; `ok=false` всегда блок, fail-open только при сбое шлюза.
